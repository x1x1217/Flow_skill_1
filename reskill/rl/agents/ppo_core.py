...
class MLPGaussianActor(Actor):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation, std_fixed=0.1):
        super().__init__()
        log_std = -0.5 * np.ones(act_dim, dtype=np.float32)
        #std = std * np.ones(act_dim, dtype=np.float32)
        #self.std = torch.nn.Parameter(torch.as_tensor(0.5 * np.ones(act_dim, dtype=np.float32)))
    ...
    def _distribution(self, obs):
        ...
        std = torch.clip(torch.exp(self.log_std), 0.2, 1.0) #
        #std = torch.exp(self.log_std).detach()
