...
class ModeClassifier(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim=256, hidden_layers=2, num_classes=2):
        super().__init__()
        layers = []
        dim = input_dim
        for _ in range(hidden_layers):
            layers.extend([torch.nn.Linear(dim, hidden_dim), torch.nn.ReLU()])
            dim = hidden_dim
        layers.append(torch.nn.Linear(dim, num_classes))
...
def load_mode_classifier(path):
    ...
    model.eval()
    id_to_label = checkpoint.get("id_to_label", {0: "push", 1: "pick"})
    id_to_label = {int(k): str(v) for k, v in id_to_label.items()}
    return {
        "model": model,
        "mean": torch.as_tensor(checkpoint["feature_mean"], dtype=torch.float32, device=device),
        "std": torch.as_tensor(checkpoint["feature_std"], dtype=torch.float32, device=device),
...
@torch.no_grad()
def predict_mode(mode_classifier, obs, condition):
    if mode_classifier is None:
        return "unknown", {"push": 0.0, "pick": 0.0}
    ...
    x = (x - mode_classifier["mean"]) / mode_classifier["std"]
    probs = torch.softmax(mode_classifier["model"](x), dim=1)[0]
    pred = int(torch.argmax(probs).item())
    label = mode_classifier["id_to_label"].get(pred, "unknown")
    return label, {
        "push": float(probs[0].item()),
        "pick": float(probs[1].item()),
    }


def append_jsonl(path, records):
    if path is None or len(records) == 0:
        return
...
def train(agent, latent_q_agent, chunk_critic, chunk_replay, condition_prior,
    ...
    mode_classifier = load_mode_classifier(args.mode_classifier_path)
    mode_stats_path = args.mode_stats_path
    if mode_classifier is not None and mode_stats_path is None:
        mode_stats_path = os.path.join(
            ...
            f"seed_{args.seed}",
            args.run_variant,
            "eval_episode_mode_stats.jsonl",
        )
    if proc_id() == 0 and mode_classifier is not None:
        print(f"Loaded mode classifier: {mode_classifier['path']}", flush=True)
        print(f"Writing eval mode stats to: {mode_stats_path}", flush=True)
    ...
    for epoch in tqdm(range(agent.epochs)):
        ...
        for roll in range(50):
            ...
            episode_mode_counts = {"push": 0, "pick": 0, "unknown": 0}
            episode_mode_probs = []
            episode_chunk_modes = []
            while steps < env._max_episode_steps:
                ...
                if mode_classifier is not None:
                    mode_label, mode_probs = predict_mode(mode_classifier, obs, n)
                    episode_mode_counts[mode_label] = episode_mode_counts.get(mode_label, 0) + 1
                    episode_mode_probs.append(mode_probs)
                    episode_chunk_modes.append(mode_label)
            ...
            if r > 0:
                r_time += 1
            if mode_classifier is not None:
                mode_chunk_count = int(sum(episode_mode_counts.values()))
                known_count = int(episode_mode_counts.get("push", 0) + episode_mode_counts.get("pick", 0))
                push_frac = (
                    float(episode_mode_counts.get("push", 0) / known_count)
                    if known_count > 0 else 0.0
                )
                pick_frac = (
                    float(episode_mode_counts.get("pick", 0) / known_count)
                    if known_count > 0 else 0.0
                )
                avg_push_prob = mean_or_zero([p["push"] for p in episode_mode_probs])
                avg_pick_prob = mean_or_zero([p["pick"] for p in episode_mode_probs])
                eval_mode_records.append(
                    {
                        "epoch": int(epoch),
                        "env_step": int(env_step_cnt),
                        "eval_roll": int(roll),
                        "episode_return": float(r),
                        "success": bool(episode_success),
                        "steps": int(steps),
                        "chunk_count": mode_chunk_count,
                        "known_chunk_count": known_count,
                        "mode_counts": {
                            key: int(value)
                            for key, value in sorted(episode_mode_counts.items())
                        },
                        "push_frac": push_frac,
                        "pick_frac": pick_frac,
                        "avg_push_prob": avg_push_prob,
                        "avg_pick_prob": avg_pick_prob,
                        "chunk_modes": episode_chunk_modes,
                        "return_ge_threshold": bool(r >= args.mode_stats_return_threshold),
                        "return_threshold": float(args.mode_stats_return_threshold),
                    }
                )

        if proc_id() == 0:
            append_jsonl(mode_stats_path, eval_mode_records)
            ...
            if len(eval_mode_records) > 0:
                high_records = [
                    record for record in eval_mode_records
                    if record["episode_return"] >= args.mode_stats_return_threshold
                ]
                writer.add_scalar('eval_mode/push_frac', mean_or_zero([r["push_frac"] for r in eval_mode_records]), epoch)
                writer.add_scalar('eval_mode/pick_frac', mean_or_zero([r["pick_frac"] for r in eval_mode_records]), epoch)
                writer.add_scalar('eval_mode/high_return_count', len(high_records), epoch)
                writer.add_scalar('eval_mode/high_return_push_frac', mean_or_zero([r["push_frac"] for r in high_records]), epoch)
                writer.add_scalar('eval_mode/high_return_pick_frac', mean_or_zero([r["pick_frac"] for r in high_records]), epoch)
...
def main():
    ...
    parser.add_argument('--mode_classifier_path', type=str, default=None)
    parser.add_argument('--mode_stats_path', type=str, default=None)
    parser.add_argument('--mode_stats_return_threshold', type=float, default=30.0)
    ...
    if args.prior_model == 'Flow':
        ...
        if args.mode_classifier_path is not None:
            args.run_variant += "_modestats"
